...
export default function CustomerSearch({ onSelect }: CustomerSearchProps) {
  ...
  const search = async () => {
    if (!query.trim()) {
      ...
      setLoading(true);
      try {
        const res = await customersApi.list({ limit: 50 });
        setResults(res.customers ?? []);
        setTotal(res.total ?? 0);
      } catch {
        setResults([]);
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="flex items-center justify-center py-8 gap-2 text-xs text-t3">
          ...
          <motion.div key="list" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            {results.length === 0 ? (
              <p className="text-sm text-t3 text-center py-4">
                {searched ? 'No customers found.' : 'No customers available.'}
              </p>
            ) : (
              <>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-t3 uppercase tracking-wider">
                    {searched ? `${results.length} result${results.length !== 1 ? 's' : ''}` : `All customers · ${total} total`}
                  </span>
                </div>
                {results.map((c, i) => (
                  <motion.div
